...
//new vs. old
module PlottingAndScheming.SchemeOld
    ...
    | scmObject.Atom a -> 
        ...
        | scmAtom.Symbol s ->
            let checkGlobalEnv () =
                if s = "y" then
                    ()
                let bind = env.tryFind s
                match bind with
                | Some {symbol = s; thunk = t} ->
                    if t.evaled then
                        t.value
                    else 
                        let v = eval t.value t.parent
                        if bind.IsSome then                   
                            t.value <- v
                            bind.Value.thunk.evaled <- true
                            v
                        else
                            failwith "bad binding"
                | None -> 
                    failwith ("bad lookup of symbol " + s)                    
            match block with
            | None -> 
                checkGlobalEnv ()
            | b ->
                let rec iter (parent : scmBlock option) = 
                    ...
                    | Some l ->
                        ...
                        if localEnv.IsSome then
                            ...
                            if s = "y" then
                                ()
                            let localBind = localEnv.tryFind s
...
let tabItemScheme () = 
    ...
    let txtIn = new TextBox ()
    txtIn.Height <- 165.0
    txtIn.AcceptsReturn <- true
    txtIn.AcceptsTab <- true
    txtIn.TextWrapping <- TextWrapping.Wrap
    txtIn.IsUndoEnabled <- true
    txtIn.VerticalScrollBarVisibility <- ScrollBarVisibility.Auto
    txtIn.HorizontalScrollBarVisibility <- ScrollBarVisibility.Auto
    let txtOut = new TextBox ()
    txtOut.Height <- 165.0
    txtOut.AcceptsReturn <- true
    txtOut.AcceptsTab <- true
    txtOut.TextWrapping <- TextWrapping.Wrap
    txtOut.IsUndoEnabled <- true
    txtOut.VerticalScrollBarVisibility <- ScrollBarVisibility.Auto
    txtOut.HorizontalScrollBarVisibility <- ScrollBarVisibility.Auto
    //to do:  scroll viewer
    let stack = new StackPanel ()
    stack.Orientation <- Orientation.Horizontal
    //button for viewing tokens
    let btn = new Button ()
    btn.Height <- 25.0
    btn.Content <- "tokenize"
    btn.Click.Add (printResults txtIn txtOut)
    stack.Children.Add btn |> ignore
    //button for viewing the heap
    let btn = new Button ()
    btn.Height <- 25.0
    btn.Content <- "heapify"
    btn.Click.Add
        (fun _ -> 
            ...
            try 
                let tokens = tokenize str |> scmTokens.create
                txtOut.Text <- buildHeap tokens |> printHeap
            with 
                | ex -> 
                    MessageBox.Show(ex.ToString()) |> ignore
            )
    ...
    btn.Click.Add 
        (fun _ -> 
            let str = txtIn.Text
            try 
                let evaled = evalString str
                let dumpHeap = printHeap evaled
    ...
    stack.Children.Add btn |> ignore
    pnl.Children.Add txtIn |> ignore
    pnl.Children.Add stack |> ignore
    pnl.Children.Add txtOut |> ignore
    tabItem.Content <- pnl
    tabItem
