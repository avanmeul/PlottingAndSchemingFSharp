...
//new vs. old
module PlottingAndScheming.VectorOld
...
7) originf needs LISPy equivalent
*)
    ...
    static member create (picker : palettePicker) level generations = {
        ...
        level = 
            if level = 0 then
                picker.inc ()
                level
            else if level >= generations then                
                failwith "level must be less than the generations"
            else level; }
...
let seed, rules = 
    ...
    let anglePlus60 angle flip = angle + (degrees60 * flip)
    let angleLess60 angle flip = angle - (degrees60 * flip)
    let fivePiOver6 = 5.0 * Math.PI / 6.0
    let anglePlus5PiOver6 angle flip = angle + (fivePiOver6 * flip)
    let angleLess5PiOver6 angle flip = angle - (fivePiOver6 * flip)
    let lenDiv3sqrt3 len = len / (3.0 * sqrt 3.0)
    let angleLessPiHalves angle flip = angle - (flip * Math.PI / 2.0)                
    let initiator = [ 
        //(list identity project1.2 project3.4 1 1)
         {  lenf = vecLen.BuiltIn identity;
            ...
            flipAngle = vecFlip.BuiltIn 1.0;
            flipRules = vecFlip.BuiltIn 1.0;
            }]  
    let generator = [
                ...
                let createRules rules = 
                    let rec iter rules records =
                        if funNull rules then
                            ...
                            let flipa = (fromScheme flipAngle) :?> int |> double                     
                            let rule = funCdr rule
                            let flipRules = funCar rule
                            let flipr = (fromScheme flipRules) :?> int |> double
                            let record = 
...
let vectorFractal (plotobj : plotObject) =
    ...
    let twoDvectorFractal 
        ...
        let rec mapVector 
            ...
            angle 
            (origin : double * double) 
            (flipAngleFactor : double)
            (flipRulesFactor : double)
            (generation : double) //to do:  change to int
        ...
        mapVector 
            ...
            -1.0 //generation
            (List.rev seed) 
    let quitf (generations : double) = 
        if generations < 0.0 then
            fun (len : double) (generation :double) -> len < generations
        else 
            fun (len : double) (generation :double) -> generation = generations    
    let win = new Window()
    win.Title <- "vector test"
...
let vectorTabItem () =
    ...
    //new vs. old
    tabItem.Name <- "tabVectorOld"
    //new vs. old
    tabItem.Header <- "vector (old)"
    let pnl = new StackPanel ()
    let cbx = new ComboBox ()
    cbx.DisplayMemberPath <- "name"
    //new vs. old
    let xml = XDocument.Load("vectorOld.xml")    
